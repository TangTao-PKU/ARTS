...
class Pose2Mesh(nn.Module):
    def __init__(self, num_joint, embed_dim=256, SMPL_MEAN_vertices=osp.join(BASE_DATA_DIR, 'smpl_mean_vertices.npy')):
        ...
        self.mcca1 = CrossAttentionBlock(q_dim=512, k_dim=512, v_dim=512, kv_num = cfg.DATASET.seqlen, num_heads=8, mlp_ratio=4., qkv_bias=True,
                                                drop=0., attn_drop=0., drop_path=0.2, has_mlp=True)
        self.mcsa1 = Transformer(2, 512, 512*4,length=cfg.DATASET.seqlen)
        self.mcca2 = CrossAttentionBlock(q_dim=512, k_dim=512, v_dim=512, kv_num = cfg.DATASET.seqlen, num_heads=8, mlp_ratio=4., qkv_bias=True,
                                                drop=0., attn_drop=0., drop_path=0.2, has_mlp=True)
        self.mcsa2 = Transformer(2, 512, 512*4,length=cfg.DATASET.seqlen)
    ...
    def forward(self, joints, img_feats, is_train=True, J_regressor=None):
        ...
        img_feats_cross = self.mcca1(motion, img_feats_trans, img_feats_trans)  # B 16 512
        img_feats_cross = self.mcsa1(img_feats_cross)
        img_feats_cross = self.mcca2(motion, img_feats_cross, img_feats_cross)
        img_feats_cross = self.mcsa2(img_feats_cross)
        ...
        # 假设已知的20670维向量
        Output = inv_bone2mesh.transpose(0,1)  # 20670维向量
        # 假设给定的10×20670维矩阵
        Coefficient = self.shapedirs.to(torch.float).cuda()  # 20670×10维矩阵
        # 使用 PyTorch 中的线性方程求解函数求解线性方程组
        X = torch.linalg.lstsq(Coefficient, Output)
        Beta = X.solution.transpose(0,1)
        inv_mesh2shape = Beta[:,None,:].repeat(1,cfg.DATASET.seqlen,1)
        # 实际上线性求解有error
        # O_rec = torch.matmul(Coefficient, Beta)
        # error = torch.norm(O_rec - Output)    # 默认求2次范数
        # print(error)                          # tensor(39.6884)
        # print(torch.sum(Output))                # tensor(3091.7029
