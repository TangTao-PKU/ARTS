import os, sys
sys.path.append('/data0/tangt/models/PMCE/Seq2Seq/lib')
...
class BilinearLayer(nn.Module):
    def __init__(self, dim_x1, dim_x2, dim_y):
        super(BilinearLayer, self).__init__()
        # 初始化需要学习的参数矩阵 A
        self.A = nn.Parameter(torch.randn(dim_y, dim_x1, dim_x2))

    def forward(self, x1, x2):
        # x1: (batch_size,seqlen, dim_x1)
        # x2: (batch_size,seqlen, dim_x2)

        Ax2 = torch.einsum('yxz,zsb->yxsb', self.A, x2.T)
        
        y = torch.einsum('yxsb,xsb->ysb', Ax2, x1.T)
...
class Pose2Mesh(nn.Module):
    def __init__(self, num_joint, embed_dim=256, SMPL_MEAN_vertices=osp.join(BASE_DATA_DIR, 'smpl_mean_vertices.npy')):
        ...
        self.bilinear = BilinearLayer(512, num_joint*3, 512)
    ...
    def forward(self, joints, img_feats, is_train=True, J_regressor=None):
        ...
        joint_feature = joints_seq.view(batch_size, 16, -1)

        img_feats_trans = self.proj(img_feats)              # B 16 512
        
        img_feats_trans = self.bilinear(img_feats_trans, joint_feature)
