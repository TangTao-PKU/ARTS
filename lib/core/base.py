...
def prepare_network(args, load_dir='', is_train=True):
    ...
    if is_train or load_dir:
        ...
        # model = models.PMCE.get_model(17,64,2) # 模型初始化
        model = model.cuda()
        pose = torch.randn(1, 16, main_dataset.joint_num, 2).cuda()
        ...
        with torch.no_grad():
            for i, (inputs, targets, meta) in enumerate(loader):
                ...
                if i == 50:
                    ...
                    mesh.export('/data0/tangt/models/PMCE/output/gt_obj_3dpw.obj')

                    pred_obj = pred_mesh[16].cpu().numpy()
                    mesh = trimesh.Trimesh(vertices=pred_obj, faces=mesh_model.face, process=False)
                    mesh.export('/data0/tangt/models/PMCE/output/pred_obj_3dpw.obj')
                    # import pdb; pdb.set_trace()
