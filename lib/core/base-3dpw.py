...
class Trainer:
    ...
    def train(self, epoch):
        ...
        for i, (inputs, targets, meta) in enumerate(batch_generator):
            ...
            loss1, loss2, loss4, loss5, loss6 = self.loss[0](pred_mesh, gt_mesh[:,cfg.DATASET.seqlen // 2], val_mesh[:,cfg.DATASET.seqlen // 2]),  \
                                         self.normal_weight * self.loss[1](pred_mesh, gt_mesh[:,cfg.DATASET.seqlen // 2]), \
            ...
            init_smpl_pose_loss, init_smpl_shape_loss = self.loss[6](init_axis,\
                                                                    ...
                                                                    gt_smplpose[:,cfg.DATASET.seqlen // 2],\
                                                                    gt_smplshape[:,cfg.DATASET.seqlen // 2],\
                                                                    mask_3d=None)
            ...
            if epoch > self.edge_add_epoch:
                loss3 = self.edge_weight * self.loss[2](pred_mesh, gt_mesh[:,cfg.DATASET.seqlen // 2])
                loss += loss3
    ...
    def test(self, epoch, current_model=None):
        ...
        with torch.no_grad():
            for i, (inputs, targets, meta) in enumerate(loader):
                ...
                # save_obj
                if i == 50:
                    gt_obj = gt_mesh[16,cfg.DATASET.seqlen // 2].cpu().numpy()
                ...
                j_error, s_error = self.val_dataset.compute_both_err(pred_mesh, gt_mesh[:,cfg.DATASET.seqlen // 2], pred_pose, gt_pose3d)
