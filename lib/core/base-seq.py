...
class Trainer:
    def __init__(self, args, load_dir):
        ...
        # self.val_reg3dpose = torch.ones(16,16,17,1).cuda()
    ...
    def train(self, epoch):
        ...
        for i, (inputs, targets, meta) in enumerate(batch_generator):
            ...
            # pred_pose由mesh回归
            pred_mesh = smploutput[-1]['verts']
            pred_pose = torch.matmul(self.J_regressor[None, None, :, :], pred_mesh * 1000)
            ...
            loss1, loss4, loss5, loss6 = self.loss[0](pred_mesh, gt_mesh, val_mesh),  \
            ...
            loss = loss1 + loss4 + loss5 + loss6 + mid_smpl_loss
            ...
            if i % self.print_freq == 0:
                ...
                init_loss = loss1.detach()
                smpl_loss = mid_smpl_loss.detach()
                loss6 = loss6.detach()
                pa_loss = loss1.detach()
