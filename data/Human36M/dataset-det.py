...
from aug_utils import affine_transform, j2d_processing, j3d_processing, flip_2d_joint

from funcs_utils import save_obj
import joblib
from _img_utils import split_into_chunks_pose, split_into_chunks_mesh
# from _img_utils import split_into_chunks
...
class Human36M(torch.utils.data.Dataset):
    def __init__(self, mode, args):
        ...
        if self.data_split == 'test':
            ...
            # det_2d_data_path = osp.join(cfg.data_dir, dataset_name, 'mesh_det_h36m.pkl')
            # with open(det_2d_data_path, 'rb') as file:
            #     self.datalist_pose2d_det = pickle.load(file)
            det_2d_data_path = osp.join(self.data_path, 'Human36M_test_det_joint_2d.json')
            self.datalist_pose2d_det = self.load_pose2d_det(det_2d_data_path, skip_img_path)
            ...
            det_2d_data_path = osp.join(self.data_path, 'Human36M_train_det_joint_2d.json')
            self.datalist_pose2d_det_train = self.load_pose2d_det(det_2d_data_path, skip_img_path)
            print("Check lengths of annotation and detection output: ", len(self.datalist), len(self.datalist_pose2d_det_train))
        self.seqlen = cfg.DATASET.seqlen
        self.stride = cfg.DATASET.stride
        self.img_names = [data['img_name'] for data in self.datalist]
    ...
    def load_data(self):
        ...
        # img_db = sorted(img_db, key=lambda x: x['img_name'])
        ...
        for aid in db.anns.keys():
            ...
            if img_name[:-12] == 's_09_act_05_subact_02_ca_0' or img_name[:-12] == 's_09_act_10_subact_02_ca_0' or img_name[:-12] == 's_09_act_13_subact_01_ca_0' or img_name[:-12] == 's_11_act_02_subact_02_ca_0' or img_name[:-12] == 's_11_act_06_subact_02_ca_0':
                continue
    ...
    def get_single_item(self, idx):
        ...
        # img_names = [data['img_name'] for data in self.datalist]
        # print(img_names)
        seq_data = copy.deepcopy(self.get_sequence(start_index, end_index, self.datalist))
        joint_imgs = []
        img_features = []
        ...
        for num, data in enumerate(seq_data):
            ...
            if self.input_joint_name == 'human36':
                joint_cam_h36m, joint_img_h36m = data['joint_cam'].copy(), data['joint_img'][:, :2].copy()
                root_coord = joint_cam_h36m[:1]
            ...
            joint_img, trans = j2d_processing(joint_img.copy(), (img_shape[1], img_shape[0]), bbox, rot, 0, None)
            if not cfg.DATASET.use_gt_input:
                joint_img = self.replace_joint_img(single_idx, img_id, joint_img, tight_bbox, trans, img_name)
            if flip:
