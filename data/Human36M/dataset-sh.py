...
class Human36M(torch.utils.data.Dataset):
    def __init__(self, mode, args):
        ...
        self.human36_eval_joint = (1, 2, 3, 4, 5, 6, 8, 10, 11, 12, 13, 14, 15)
        ...
        if self.data_split == 'test':
            det_2d_data_path = osp.join(self.data_path, 'Human36M_test_sh_joint_2d.json')
            ...
            if self.input_joint_name == 'human36':
                det_2d_data_path = osp.join(self.data_path, 'Human36M_train_sh_joint_2d.json')
    ...
    def get_single_item(self, idx):
        ...
        for num in range(self.seqlen):
            ...
            if cfg.MODEL.name == 'PMCE':
                ...
                # default valid
                neck_idx = self.human36_joints_name.index('Neck')
                joint_cam_1 = joint_cam[:neck_idx]
                joint_cam_2 = joint_cam[neck_idx+1:]
                # print(joint_cam_1.shape)
                # print(joint_cam_2.shape)
                joint_cam = np.concatenate((joint_cam_1,joint_cam_2),axis=0)
                # print(joint_cam.shape)

                posenet_joint_cam.append(joint_cam.reshape(1, len(joint_cam), 3))
                valid = np.ones((len(joint_cam), 1), dtype=np.float32)
                joint_valid.append(valid.reshape(1, len(joint_cam), 1))
                # print(valid.shape)
        ...
        if cfg.MODEL.name == 'PMCE':
            ...
            # print(joint_imgs[0])
            # print(posenet_joint_cam[0])
            # print(posenet_joint_cam[0,:16,:2]-joint_imgs[0])
